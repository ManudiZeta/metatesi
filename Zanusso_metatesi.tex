...
\institute[Characterization of Hadronic Showers in the Belle II Electromagnetic Calorimeter - Emanuele Zanusso]{Supervisor: Prof. \textbf{Stefano Spataro}
\newline
\newline
...
\vspace{0.1cm}
	\begin{enumerate}
		\item  Bremsstrahlung and pair production process ($e^+,e^-,\gamma$) 
		\item  Interactions of hadrons with the material ($p,n,pions,...$)
	\end{enumerate}
...
\begin{frame}{The MANTRA project (PRIN2022)}
 ...
 \vspace{0.2cm}
    ...
    \item A general method to measure the $E_{\bar{n}}$ up to 10 GeV, by combining information from:
    	...
    	\end{enumerate}
	 \vspace{0.1cm}
     \item These features are common in modern general-purpose collider experiments such as \textbf{Belle II} and BESIII, which do not have a hadronic calorimeter 
...
Several channels can be selected to look at $\bar{n}$ annihilations, such as:
	...
	\vspace{0.2cm}
Several variables can be used to validate the showers shape for $\bar{n}$ identification, such as \textbf{Energy}, \textbf{Zernike Moments}, \textbf{Lateral momentum}  and \textbf{Second moment} 
\vspace{0.1cm}
	    \begin{figure}[p]
         \includegraphics[width = 0.75 \textwidth]{images/shower_ECL}
    \end{figure}
...
The list of all $\bar{n}$ candidates shows two structures in the MC momentum distribution:
		\begin{figure}[p]
		   	\includegraphics[width = 0.42\textwidth]{images/signal_sample/pRecoil_nmcP_isr.pdf}
			\includegraphics[width = 0.42\textwidth]{images/signal_sample/pRecoil_nmcP_nog.pdf}
		\end{figure}		
	 	\begin{itemize}
		{
			...
			\item \textcolor{red}{Is there a problem in the MC association?}
			}
			\end{enumerate}
		}		
		\end{itemize}
...
\begin{frame}{The recoil and the $\bar{n}$ momentum}
 	\begin{itemize}
		{
			\item MC truth can be imposed by selecting only MC associated $\bar{n}$ candidates from the $\bar{n}$ list (\inlineimg{images/logos/red} $\bar{n}$ MC truth ID )
...
\begin{columns}  
  \begin{column}{0.50\textwidth}
     {
    \item \textbf{Energy} of the cluster, \textbf{E1E9} and \textbf{E9E21} 
    \vspace{0.5cm}
    \item \textbf{Lateral momentum} and \textbf{Second moment}:
    
		\vspace{0.3cm}
		
		$C_{LM} = \frac{ \sum\limits_{i=2}^n E_i r_i^2} {  E_0 r_0^2 + E_1 r_0^2+ \sum\limits_{i=2}^n E_i r_i^2}$, with $r_0 \simeq 5$ cm
		
		\vspace{0.3cm}
		
		$C_{SM} = \frac{ \sum\limits_{i=0}^n  E_i r_i^2} { \sum\limits_{i=0}^n E_i}$  
   ...
   \begin{column}{0.50\textwidth}
  		\begin{figure}
			\includegraphics[width = 0.75\textwidth]{images/cluster/cluster}
		\end{figure}
    \end{column}
    \end{columns}
...
\begin{itemize}
\item Channel  $e^+ + e^-  \rightarrow p + \bar{n} + \pi^- + (\gamma_{ISR})$ has been studied and the recoil system is correctly reconstructed from the secondary background 

\vspace{0.2cm}

\item Reconstructed $\bar{n}$ variables, such as momentum and shower-shape variables, should be further investigated:

\begin{enumerate}[(a)]
\item \textcolor{red}{Cluster split-offs with the ECL and pre-showering occurring within the TOP may affect the shower shape distributions}
\item \textcolor{red}{A wrong Monte Carlo association may be present as a consequence}
...
\item 1C kinematic fit can be possibly adopted during Data/MC comparison, in order to improve the recoil momentum resolution
...
$q \bar{q}$ cocktail with $341 \ M$ events (Luminosity: $215 \ fb^{-1}$) generated with Pythia8


\vspace{0.2cm}
\item Only the $p$ and $\pi^-$ are used to build the recoil vector (NO ISR for the moment) $\rightarrow$ \textcolor{red}{Can the signal be correctly reconstructed among the other background channels?}
\vspace{0.1cm}
\item Following selections have been applied:

\begin{enumerate}
\item PID selection on $p$ and $\pi^-$ from the MC truth
\item 0 GeV < recoil mass < 2 GeV
...
\item Study the shower shape variables in a selected recoil mass region, replacing the MC truth selections with data real ones
...
\begin{columns}  
  \begin{column}{0.50\textwidth}
	...
	\end{enumerate}
	\vspace{0.4cm}
	\item A peak can be observed above the $\bar{n}$ mass $\rightarrow$ \textcolor{red}{the signal is highlighted in the MC cocktail }
     \end{itemize}
     }
   \end{column}
...
\end{figure}
{ \small
Background must be further suppressed $\rightarrow$ \textcolor{red}{sideband method for background subtraction}
}
\end{frame}
...
\item The recoil mass is correctly reconstructed using both Monte Carlo based selections and real cuts, in the presence of other background channels
...
Analysis of a $\Lambda \rightarrow p + \pi^- \ (\bar{\Lambda} \rightarrow \bar{p} + \pi^+)$ sample shows that:
	...
	\end{figure}
\textcolor{red}{Could the Data/MC agreement observed for $\bar{p}$ also be expected for $\bar{n}$?}
\end{frame}
...
\begin{enumerate}
\item Study the cluster split-off and pre-showering effects using a particle gun, evaluating whether they can affect the Monte Carlo association
\vspace{0.2cm}
\item Study the complete cocktail production and attempt to further suppress the background
\vspace{0.2cm}
...
PID probability defined as:
	...
	\end{figure}	
		...
		\begin{figure}
     	   \includegraphics[scale=0.60, angle=0]{images/backup/PID_selections}	
	\end{figure}	
\end{frame}
%----------------------------------------------------------------------------------------
...
%----------------------------------------------------------------------------------------
\begin{frame}[allowframebreaks,noframenumbering, plain]{cluster LAT \& cluster Second Moment}

	\begin{figure}
     	   \includegraphics[scale=0.20, angle=0]{images/backup/ClusterLat}	
	\end{figure}	
