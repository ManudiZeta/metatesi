...
\begin{frame}[t,noframenumbering]{Summary}
    \tableofcontents
\end{frame}
...
The $\bar{n}$ plays a key role in several physics measurements, such as: 
    ...
    \begin{itemize}    
    \item The neutron e.m. form factor study in $e^+ + e^- \rightarrow n + \bar{n}$ annihilation \cite{ablikim2021}
        	\begin{figure}[p]
     	   \includegraphics[scale=0.18, angle=0]{images/feynman-eN.png}
	\end{figure}
  
    \item The hyperon decay channels which involved $\bar{n}$, such as:
	\begin{center}
    ...
    \item Other typical B-factories decay channels which involved $\bar{n}$, such as:
    	\begin{center}
...
The $\bar{n}$ interacts with matter primarily via strong nuclear force 
    ...
    \item Hadronic ($\pi^+$, $\pi^- $) and electromagnetic ($\pi^0$) showers are generated within the detectors
    \item Since annihilation stars are produced, both forward and backward directions are involved
...
Different processes occur for e.m. (1) and hadronic (2) showers:
	\begin{enumerate}
		\item  $\pi^0 \rightarrow \gamma \gamma$ and bremsstrahlung process ($e^+,e^-,\gamma$)
...
\begin{columns}
	\begin{column}{0.55\textwidth}
    		\begin{itemize}    
			\item About the 95\% of the hadronic shower is contained within a cylinder of radius $\lambda_{had}$ ($\sim$ 44.12 cm in CsI)
			\item About the 90\% of the e.m. shower is contained within a cylinder of radius $R_M$ ($\sim$ 3.6 cm in CsI)
	    	\end{itemize}
	\end{column}
	
	\begin{column}{0.45\textwidth}
		\begin{figure}[p]
		   \includegraphics[scale=0.35, angle=0]{images/gamma_vs_hadronic.png}
		\end{figure}
	\end{column}
\end{columns}
\end{frame}
...
Several channels can be selected to look at $\bar{n}$ annihilation, such as:
	\begin{itemize}
		\item $e^+ + e^- \rightarrow J/\Psi \rightarrow p + \bar{n} + \pi^-$ (BESIII)
		\item $e^+ + e^- + \gamma_{ISR} \rightarrow X \rightarrow p + \bar{n} + \pi^-$ (Belle II)
		\item $\bar{\Lambda}_c \rightarrow  K_s^0 + \pi^0 +\bar{n} $ 
	\end{itemize}
	
Several variables can be used to distinguish their clusters, such as:
    	\begin{itemize}    
		\item clusterLAT, clusterSecondMoment, clusterZernikeMoment, clusterNHits...
    	\end{itemize}
	
	\begin{figure}[p]
     	   \includegraphics[scale=0.16, angle=0]{images/ClusterLAT.png}
	   \includegraphics[scale=0.35, angle=0]{images/gamma_vs_hadronic.png}
	\end{figure}
...
\textbf{M}easuring \textbf{A}nti-\textbf{N}eutron: \textbf{T}agging and \textbf{R}econstruction \textbf{A}lgorithm:
    ...
    \item A general method to measure the $E_{\bar{n}}$ up to 10 GeV, by combining information from:
    	...
    	\begin{enumerate}
    		\item A  detector with high time resolution ($<100ps$), like a T.O.F. detector
		\item An electromagnetic calorimeter
		\item A muon system (alternating layers of active material and high-Z absorber)
    	\end{enumerate}
     \item These features are common in modern general-purpose collider experiments such as \textbf{Belle II} and BESIII
     \item For MANTRA project, only signals from (1) and/or (2) are taken into account. In this thesis only (2) signals are studied
    \end{itemize}
\end{frame}

%------------------------------------------------
\begin{frame}{The MANTRA project}
The measurement of the energy of the anti-neutrons is a two-step process:
    	\begin{enumerate}
    		\item $\bar{n}$ identification via its induced shower vs ones from by photons or other hadrons
		\item Combine the signals from (1) and (2) to reconstruct the $\bar{n}$'s energy
...
\begin{columns}[T] % T = top alignment
    ...
    \begin{column}{0.55\textwidth}
        ...
        \begin{itemize}
            ...
            \item Peak Lumi $\sim 5.1 \times 10^{34} cm^{-2} s^{-1}$ (Dec 2024)
        \end{itemize}
    \end{column}
...
\makesection{Cluster variables}

\begin{frame}{n vs gamma list in cluster variables}
...
\setbeamertemplate{footline}{} % rimuove numeri in basso
\bibliography{references}  % senza .bib
\end{frame}
