...
%------------------------------------------------
\begin{frame}{Anti-neutrons in astrophysical experiments}
The $\bar{n}$ also plays a key role in several astro-physics measurements, such as:
\vspace{0.2cm}
    \begin{itemize}    
    \item Studying $\bar{n}$ - anti-hyperon potential to improve the understanding of the equation of state of the neutron stars \cite{burgio2021}
    \vspace{0.1cm}
...
\begin{itemize}
    \item This latter decays almost exclusively into entangled couples of $B \ \bar{B} \rightarrow$ B-factory
    \item Several goals: flavour physics, BSM, B and charm mesons spectroscopy etc...
\end{itemize}
...
The $\bar{n}$ interacts with matter primarily via strong nuclear force 
    ...
    \item Hadronic and electromagnetic showers are produced by $\pi^\pm$ interactions and $\pi^0 \rightarrow \gamma\gamma$ decays, respectively, within the ECL 
    \item Since "Annihilation stars" are produced, showers develop in both forward and backward directions
...
\vspace{0.1cm}
    	\begin{enumerate}
    		\item $\bar{n}$ identification via its induced ECL clusters and correlation to the initial energy
		\item Combine the signals from (1) and (2) to reconstruct the $\bar{n}$ energy, in cases of backscatter or pre-annihilation
		\vspace{0.1cm}
		\begin{itemize}
...
%------------------------------------------------------------------------------------------------
\makesection{Preliminary study via signal MC sample}
%------------------------------------------------------------------------------------------------
...
\begin{enumerate}
	...
	\begin{enumerate}[(a)]
			\item Recoil identification from the two-body system $p + \pi^-$ (cleaner channel)
			\item Study of the kinematic recoil variables (momentum, angles, energy, etc...)
	\end{enumerate}
...
\item The analyzed channel is:
  ...
  \begin{center}
    $e^+ + e^-  + \gamma_{ISR} \rightarrow p + \bar{n} + \pi^-$ (Phokhara+evt\_gen generator)
  \end{center}
\end{itemize}
  
\begin{columns}[T]
  \begin{column}{0.45\textwidth}
The reconstructed particles are (cuts and selections in backup):
    \begin{enumerate}[(a)]
      \item $vpho \rightarrow p + \gamma_{ISR} + \pi^-$, where $vpho$ is a fake particle, mimicking the recoil system 
      \item $\bar{n}$ candidates list used to compare its variables with those of the recoil
    \end{enumerate}  
\end{column}

  \begin{column}{0.50\textwidth}
    \begin{figure}[p]
         \includegraphics[scale=0.22, angle=0]{images/topoana_100k}
    \end{figure}
  \end{column}
...
\begin{columns}[T] % T = top alignment
    % Colonna sinistra
	 	...
	 	\begin{itemize}
		{
			...
			\vspace{0.3cm}
			\\ $\rightarrow$ reconstructed $\bar{n}$ variables can be compared with the reconstructed recoil variables ($p, \ \theta$)
		}
		\end{itemize}
	\end{column}
...
\begin{frame}{The recoil and the $\bar{n}$ momentum (1b)}
...
\end{frame}
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}{$\bar{n}$ vs recoil vector correlation (1b)}
	\begin{columns}[T] % T = top alignment
    % Colonna sinistra
...
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}{$\bar{n}$ vs recoil vector residuals (1b)}
	\begin{columns}[T] % T = top alignment
    % Colonna sinistra
    	\begin{column}{0.30\textwidth}
	 	\begin{itemize}
...
%-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}{KInematic Fit over the recoil mass (1c)}
A 1C kinematic fit can possibly be used to add a constraint and improve the agreement in $p$ and $\theta$
	 	...
	 	\begin{itemize}
		{
			...
			\item A slightly improvement can be observed in in $p_{recoil}$ vs MC $p_{\bar{n}}$
		}
		\end{itemize}
		\begin{figure}
...
%-----------------------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}{$\bar{n}$ ECL cluster variables (1d)}
Cluster variables can be studied to distinguish $\bar{n}$ from other neutral particles:

\begin{itemize}
\begin{columns}  
  \begin{column}{0.50\textwidth}
     {
    \item \textbf{clusterE}, \textbf{clusterE1E9} and \textbf{clusterE9E21} ($E_{min} = 20$ GeV)
     \item \textbf{clusterAbsZernikeMoment51}: $|Z_{51}|$ 
     }
   \end{column}
   
   \begin{column}{0.40\textwidth}
  		\begin{figure}[p]
		  	\includegraphics[scale=0.10, angle=0]{images/cluster/zernike.png}
			\includegraphics[scale=0.13, angle=0]{images/cluster/cluster}
		\end{figure}
    \end{column}
    \end{columns}
   
    \item \textbf{clusterLAT}:  lateral energy distribution, defined as:  $S = \frac{ \sum\limits_{i=2}^n \omega_i E_i r_i^2} { \omega_0 E_0 r_0^2 + \omega_1 E_1 r_1^2+ \sum\limits_{i=2}^n \omega_i E_i r_i^2}$    
    \item \textbf{clusterSecondMoment}: second moment S, defined as:  $ S = \frac{ \sum\limits_{i=0}^n \omega_i E_i r_i^2} { \sum\limits_{i=0}^n \omega_i E_i}$  
...
\begin{frame}{$\bar{n}$ ECL cluster variables (1d)}
 \begin{figure}
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterE_gamma}
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterE1E9_gamma}
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterE9E21_gamma}
	
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterAbsZernikeMoment51_gamma}
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterLAT_gamma}
	\includegraphics[width=0.31\textwidth]{images/cluster/mcPDG!=22/clusterSecondMoment_gamma}
\end{figure}
...
\item 1C kinematic fit can be possibly adopted during MC/Data comparison, in order to reduce the uncertainty on the recoil momentum
...
\begin{itemize}
\item Study of uubar continuum using the following MC cocktail sample:
\begin{center}
	$/belle/collection/MC/MC16rd\_proc16\_chunk1\_uubar\_4S\_v1$
\end{center}
\vspace{0.2cm}
\item To obtain a cleaner channel, only the $p$ and $\pi^-$ are used to build the recoil vector (ISR neglected for the moment)
...
\begin{columns}  
  ...
  \begin{column}{0.50\textwidth}
	...
	\item nRoeCharged == 0 and alpha<0.35 (additional angular cut on the best candidate) \inlineimg{images/logos/green} 
...
\begin{columns}  
  ...
  \begin{column}{0.50\textwidth}
     {
     \begin{itemize}
     \item Other "Real selections" can be applied as well, with (b), such as: \cite{longo2025}
     \vspace{0.2cm}
\\  protonID > 0.9 and pionID > 0.1 and
\\ dr < 1 and abs(dz) < 3 (from IP)  \inlineimg{images/logos/cyan}
	
	\vspace{0.2cm}
	\item To maximize purity, a recoil mass zoom in the range (0.8-1.1) GeV can be applied to study the recoil variables
...
\backupbegin
\begin{frame}[allowframebreaks,noframenumbering, plain]{Selections/cuts on clean MC sample}
\vspace{0.6cm}
\begin{columns}[T] % T = top alignment
    % Colonna sinistra
    	\begin{column}{0.50\textwidth}
