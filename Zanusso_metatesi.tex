...
\vspace{0.1cm}
		...
		\vspace{0.2cm}
		\begin{itemize}
			\item If only $\pi^0$ ($\sim 5 \%$) are produced energy is all contained in the calorimeter, the shower is fully reconstructed
			\vspace{0.2cm}
...
\begin{frame}{The recoil and the $\bar{n}$ momentum}
	 	...
	 	\begin{itemize}
		{
			...
			\begin{enumerate}[(a)]
			{
			\item \textcolor{red}{Could a cluster split-off introduce the left peak?}
			\item \textcolor{red}{Could pre-showering affect the MC association?}
			}
			\end{enumerate}
		}		
		\end{itemize}
...
\begin{columns}  
  \begin{column}{0.35\textwidth}
     {
     \begin{enumerate}
     \item Could several cluster (split-off) be mis-identified as $\bar{n}$ primary cluster during reconstruction?
...
\begin{enumerate}[(a)]
\item \textcolor{red}{MC association affected by pre-showering within the TOP (?)}
\item \textcolor{red}{Secondary cluster (split-off) mis-identified in the best candidate (?)}

\vspace{0.2cm}

$\rightarrow$ \textcolor{red}{This could be addressed by testing the ECL response with a $\bar{n}$ particle gun generator}
\end{enumerate}
...
\begin{columns}  
  \begin{column}{0.50\textwidth}
	...
	\item No additional charged tracks in the event and alpha<0.35 (additional angular cut on the closest candidate) \inlineimg{images/logos/green} 
	\end{enumerate}
	\vspace{0.4cm}
...
\begin{columns}  
  \begin{column}{0.50\textwidth}
	...
	\vspace{0.2cm}
	\item To select the signal, a recoil mass cut in the range (0.8-1.2) GeV can be applied to study the shower shapes variables
