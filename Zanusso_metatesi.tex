...
\begin{columns}[T] % T = top alignment
    % Colonna sinistra
	 	...
	 	\begin{itemize}
		{
			...
			$\rightarrow$ several $\gamma$'s are mis-identified as $\bar{n}$ in reconstruction 
			\vspace{0.2cm}
...
\begin{columns}[T] % T = top alignment
    % Colonna sinistra
	 	...
	 	\begin{itemize}
		{
			...
			\item For a $(LUMI)$ real data, $t(TOT events)$ are expected
		}
		\end{itemize}
	\end{column}
...
\begin{columns}  
  ...
  \begin{column}{0.35\textwidth}
     {
     \begin{itemize}
     \item $\bar{n}$ clusters mainly involved 15 or more crystals
   ...
   \begin{column}{0.55\textwidth}
 \begin{figure}
	\includegraphics[width=0.95\textwidth]{images/cluster/clusterNHits}
\end{figure}
    \end{column}
    \end{columns}
...
\end{center}
\item 2300 jobs have been submitted to the grid, with the following online cuts:
\begin{enumerate}
\item  $p\_mcPDG == 2212$ and $pi\_mcPDG == -211$ and $mRecoil > 0 GeV$ and $mRecoil < 2 GeV$
\item Best candidate selected with RankByLowest method over $\alpha$
\item In order to have a cleaner channel, only $p$ and $\pi^-$ compose the recoil vector (ISR neglected)
\end{enumerate}

\begin{enumerate}[(a)]
\item Find the signal above the $\bar{n}$ mass $\sim 0.939 \ GeV$
\item Zoom around this value and look at the same variables of the sample case 
\end{enumerate}
