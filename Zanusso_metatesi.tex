...
Different processes occur for electromagnetic and hadronic interactions:
\vspace{0.1cm}
	\begin{enumerate}
		\item  Bremsstrahlung and pair production process ($e^+,e^-,\gamma$) 
		\item  Interactions of hadrons with the material ($p,n,p^{\pm}...$) and $\pi^0 \rightarrow \gamma \gamma$
	\end{enumerate}
...
\begin{columns}  
  \begin{column}{0.50\textwidth}
    ...
    \item \textbf{Lateral momentum} and \textbf{Second moment}:
		...
		$C_{LM} = \frac{ \sum\limits_{i=2}^n \omega_i E_i r_i^2} { \omega_0 E_0 r_0^2 + \omega_1 E_1 r_1^2+ \sum\limits_{i=2}^n \omega_i E_i r_i^2}$
		
		\vspace{0.3cm}
		
		$C_{SM} = \frac{ \sum\limits_{i=0}^n \omega_i E_i r_i^2} { \sum\limits_{i=0}^n \omega_i E_i}$  
...
\item 1C kinematic fit can be possibly adopted during MC/Data comparison, in order to improve the recoil momentum resolution
...
\begin{enumerate}
\item Particles ID selection on $p$ and $\pi^-$ from the MC truth
\item 0 GeV < recoil mass < 2 GeV
...
Analysis of a $\Lambda \rightarrow p + \pi^- \ (\bar{\Lambda} \rightarrow \bar{p} + \pi^+)$ sample shows that:
	...
	\end{figure}
Could the poorer Data/MC agreement observed for $\bar{p}$ also be expected for $\bar{n}$?
\end{frame}
...
\begin{enumerate}
\item Study the cluster split-off and pre-showering effects in the ECL using a particle gun, evaluating whether they can affect the Monte Carlo association
\vspace{0.2cm}
